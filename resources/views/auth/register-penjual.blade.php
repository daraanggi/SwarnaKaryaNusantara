    ...
    <div class="bg-cover bg-center min-h-screen" style="background-image: url('/images/background.jpg');">
        <div class="flex min-h-screen items-center justify-center px-6 py-10 lg:px-20">
            <div class="flex flex-col lg:flex-row w-full max-w-6xl bg-white/90 shadow-2xl rounded-2xl overflow-hidden">
                ...
                <!-- Kolom Kanan (Form Penjual) -->
                <div class="w-full lg:w-1/2 p-10 bg-[#7B5E3C] text-white">
                    <h2 class="text-3xl font-bold text-center mb-8">DAFTAR AKUN TOKO</h2>

                    {{-- GANTI route() sesuai rute submit penjual-mu --}}
                    <form method="POST" action="{{ route('register.penjual.store') }}" enctype="multipart/form-data">
                        @csrf

                        {{-- Nama Toko (kalau mau ganti menjadi penanggung jawab, biarkan name="name") --}}
                        <div class="mb-4">
                            <label for="name" class="block mb-1">Nama Toko :</label>
                        ...
                        <div class="mb-4">
                            ...
                            <input id="no_telepon" name="no_telepon" type="text" value="{{ old('no_telepon') }}" required
                                   class="w-full px-4 py-3 rounded-full text-black focus:outline-none" />
                        ...
                        {{-- Upload KTP --}}
                        <div class="mb-4">  {{-- tadinya mb-6 --}}
                            <label for="foto_ktp" class="block mb-1">Upload Foto KTP :</label>
                            <input id="foto_ktp" name="foto_ktp" type="file" accept="image/*,application/pdf"
                                class="w-full px-4 py-3  {{-- tadinya py-2 --}}
                                        rounded-full text-black bg-white/90
                                        file:mr-3 file:rounded-full file:border-0
                                        file:bg-[#7B5E3C] file:px-4 file:py-2
                                        file:font-semibold file:text-white hover:file:bg-[#6d5134]" />
                            <x-input-error :messages="$errors->get('foto_ktp')" class="mt-2 text-sm text-red-200" />
                        </div>

                        <div class="flex justify-between items-center mt-2"> {{-- tadinya mt-6 --}}
                            <p class="text-white mt-0 text-sm">
                                Sudah punya akun?
                        ...
                        <div class="-mt-1 text-left text-sm leading-tight">
                            <span class="text-white">Ingin mendaftar sebagai Pembeli?</span>
                            <a href="{{ route('register') }}" class="text-yellow-300 hover:underline ml-1">
